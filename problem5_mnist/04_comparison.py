...
# --- Grad-CAM ---
print("\nGrad-CAM 計算中...")
grad_cam_method = LayerGradCam(model, model.conv3)
gradcam_maps = []
...
for i in range(NUM_SAMPLES):
    ...
    attr = occ_method.attribute(
        ...
        sliding_window_shapes=(1, 4, 4), strides=(1, 2, 2), baselines=0
    )
    attr_np = attr.squeeze().detach().cpu().numpy()  # [28, 28]
    attr_2d = np.abs(attr_np)
    if attr_2d.max() > 0:
...
for col in range(NUM_SAMPLES):
    gray = raw_images[col].squeeze().numpy()  # [28, 28]
    original = np.stack([gray, gray, gray], axis=-1)  # [28, 28, 3]

    # 行0: 元画像
    axes[0, col].imshow(original)
    axes[0, col].set_title(f"数字: {labels[col]}", fontsize=12, fontweight='bold')
    axes[0, col].axis('off')
...
fig.suptitle('XAI 手法比較: Grad-CAM vs Integrated Gradients vs Occlusion\n'
             '(MNISTNet / MNIST)', fontsize=16, y=0.98)
plt.tight_layout()
...
if len(raw_images) > 0:
    ...
    attr_signed = attr.squeeze().detach().cpu().numpy()  # [28, 28]
    vmax = max(abs(attr_signed.min()), abs(attr_signed.max()))
    if vmax == 0:
        vmax = 1
    im = ax.imshow(attr_signed, cmap='RdBu_r', vmin=-vmax, vmax=vmax)
...
fig.suptitle('XAI 手法の特性サマリー (MNIST)', fontsize=16, y=1.01)
plt.tight_layout()
