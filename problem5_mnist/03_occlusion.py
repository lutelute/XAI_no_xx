...
- スライディングウィンドウ: (1, 4, 4), ストライド: (1, 2, 2)
...
for i, (raw_img, label) in enumerate(zip(raw_images, labels)):
    ...
    # Occlusion 計算 (1チャネル用に window と stride を調整)
    ...
    print(f"  [{i+1}/10] 数字 {label}: Occlusion 計算完了")
...
def create_occ_overlay(raw_img_tensor, attr_2d, alpha=0.5):
    ...
    colored_attr = cm.inferno(attr_2d)[:, :, :3]       # [28, 28, 3]
...
fig.suptitle('Occlusion Sensitivity マップ (MNISTNet / MNIST)', fontsize=16, y=0.98)
plt.tight_layout()
...
for i in range(5):
    ...
    fig.suptitle(f'Occlusion Sensitivity 詳細: 数字 {labels[i]} (サンプル {i})',
                 fontsize=14, y=1.02)
    plt.tight_layout()
